...
object createSingleHZeroGModel {
  ...
  def apply(x: DenseVector[Double], y: DenseMatrix[Double], z: DenseVector[Double]): CogpModel =  createSingleHZeroGModel(x.toDenseMatrix.t,y,z.toDenseMatrix.t)
   def apply(x: DenseMatrix[Double], y: DenseMatrix[Double]): CogpModel = createSingleHZeroGModel(x, y, z = x)
